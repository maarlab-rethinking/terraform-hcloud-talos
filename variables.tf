...
variable "control_plane_server_type" {
  ...
  description = <<EOF
    ...
    Possible values:
      "cx23", "cx33", "cx43", "cx53",                      # Cost-Optimized x86 (Intel/AMD)
      "cax11", "cax21", "cax31", "cax41",                  # Cost-Optimized xArm (Ampere)
      "cpx22", "cpx32", "cpx42", "cpx52", "cpx62",         # Regular Performance x86 (AMD)
      "ccx13", "ccx23", "ccx33", "ccx43", "ccx53", "ccx63" # General Purpose x86 (AMD)
  EOF
  validation {
    condition = contains([
      "cx23", "cx33", "cx43", "cx53",
...
variable "kubernetes_version" {
  ...
  default     = "1.33.7"
  description = <<EOF
    The Kubernetes version to use. If not set, the latest version supported by Talos is used: https://www.talos.dev/v1.12/introduction/support-matrix/
