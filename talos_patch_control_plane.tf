    ...
    for control_plane in local.merged_control_planes : control_plane.name => {
      ...
      cluster = {
        ...
        externalCloudProvider = {
          enabled = true
          manifests = [
            "https://raw.githubusercontent.com/siderolabs/talos-cloud-controller-manager/${regex("^v\\d+\\.\\d+", var.talos_version)}.0/docs/deploy/cloud-controller-manager-daemonset.yml"
          ]
        }
      }
    }
