...
data "talos_machine_configuration" "control_plane" {
  for_each           = { for control_plane in local.merged_control_planes : control_plane.name => control_plane }
  talos_version      = var.talos_version
...
data "talos_machine_configuration" "dummy_control_plane" {
  ...
  machine_type       = "controlplane"
  machine_secrets    = talos_machine_secrets.this.machine_secrets
  config_patches     = concat([yamlencode(local.controlplane_yaml["dummy-cp-0"])], var.talos_control_plane_extra_config_patches, local.tailscale_config_patch != null ? [local.tailscale_config_patch] : []) # Use dummy yaml + extra patches
  docs               = false
  examples           = false
}
